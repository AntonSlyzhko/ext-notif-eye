...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/unit/Espo/Modules/NotifEye)
```
...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/integration/Espo/Modules/NotifEye)
```
...
If your extension contains additional PHP packages, you also need to add `site/custom/Espo/Modules/NotifEye/vendor` to the *scanDirectories* section in *phpstan.neon* config.
...
* `site/custom/`
* `site/client/custom/`
* `site/tests/unit/Espo/Modules/NotifEye`
* `site/tests/integration/Espo/Modules/NotifEye`
...
2. Set *bundled* and *jsTranspiled* to true in `src/files/custom/Espo/Modules/NotifEye/Resources/module.json`.
3. Add `src/files/custom/Espo/Modules/NotifEye/Resources/metadata/app/client.json`
    ```json
    {
        "scriptList": [
            "__APPEND__",
            "client/custom/modules/notif-eye/lib/init.js"
        ]
    }
    ```
...
Add the library module path to `src/files/custom/Espo/Modules/NotifEye/Resources/metadata/app/jsLibs.json`

```json
{
    "some-lib": {
        "path": "client/custom/modules/notif-eye/lib/some-lib.js"
    }
}
```
